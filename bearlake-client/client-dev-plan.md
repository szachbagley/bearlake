...
| C4 | Simulator | One pinned iPhone simulator, **name recorded in Phase 0** after enumerating what is actually installed. No iOS runtime was detectable before `xcode-select` was fixed, so the runtime may need downloading. | `CLAUDE.md`'s example command names "iPhone 16", which may not exist on this machine. Phase 0 records the real name and this plan is updated to match — a guessed device name fails every build command. |
| C5 | Adding Swift files | **Files auto-add. No Xcode GUI step, no `project.pbxproj` edits.** The target uses a `PBXFileSystemSynchronizedRootGroup` over the inner `bearlake-client/` folder (`objectVersion = 77`): any `.swift` file created anywhere beneath it is compiled into the target automatically. | **This contradicts `CLAUDE.md`'s working agreement** ("New Swift files do not auto-add to the Xcode target… `project.pbxproj` is authoritative and hand-managed"), which was written for the older project format. Evidence: the `PBXSourcesBuildPhase` `files` list is *empty* and neither existing Swift file is referenced individually, yet the app builds. **Confirm with the first build in Phase 0, then amend `CLAUDE.md`.** The "never hand-edit `project.pbxproj`" half of that rule still stands and gets stronger — the file is now generated, not curated. |
...
| C10 | Scheme / product name | Keep the existing scheme and target name **`bearlake-client`** and bundle id `hansen.bearlake-client`. | `CLAUDE.md`'s example commands say `-scheme BearLake`, which does not exist. Renaming a scheme to match a doc is backwards; amend `CLAUDE.md` instead (same edit as C5). |
...
1. **Point the toolchain at Xcode** (user runs; requires `sudo`):
   `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer`
   then `sudo xcodebuild -license accept`. Verify: `xcodebuild -version` prints 26.0.1.
...
11. **Amend `CLAUDE.md`** (C5/C10): correct the "new Swift files do not auto-add" agreement, fix the `-scheme BearLake` commands to `bearlake-client`, and keep the "never hand-edit `project.pbxproj`" rule.
...
- **Two `CLAUDE.md` amendments fall out of Phase 0** (C5, C10): Swift files *do* auto-add, and the scheme is `bearlake-client`, not `BearLake`. Make them in Phase 0 so the guidance stops being wrong.
