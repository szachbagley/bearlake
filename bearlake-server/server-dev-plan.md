...
| D34 | Migration SQL in the build (added in Phase 1) | `npm run build` runs `tsc` then `scripts/copy-migrations.mjs`, copying `src/db/migrations/*.sql` into `dist/`. | `tsc` emits only JavaScript; without the copy, `npm start` boots and finds no migrations, which on a fresh Railway database would start a server against an empty schema. |
