...
| D41 | bcrypt cost configurable, low under test (added in Phase 7, refines D3/D29) | `BCRYPT_COST` env var, default 12, floor of 10 enforced in production; the test environment sets 4. | Cost-12 bcrypt (~200ms/hash) dominates the suite runtime and, run serially under load, was intermittently timing out bcrypt-heavy tests (the D39 symptom). Lowering the work factor for tests only cut the suite from ~130–280s to ~55s and removes the load-based flakiness at its root. Production strength is unchanged and guarded: a sub-10 cost fails the boot in production, and a config test asserts the default is 12. |
| D42 | Real-S3 verification deferred to Phase 8 (added in Phase 7) | The presign logic is fully covered offline (SigV4 is a local computation — signed-header/expiry/validation/auth all tested, and confirmed on the built artifact). The manual round-trip against a real bucket (actual PUT/GET, S3-side rejection of a mismatched type/size, expiry → 403) is deferred to Phase 8, where the bucket is created as part of deployment. | No AWS credentials or bucket exist in this environment, and standing up MinIO would require an `S3_ENDPOINT` override not otherwise needed. The bucket is a Phase 8 deliverable, so the real round-trip naturally belongs there. |
| D40 | `LIMIT` uses `query`, not `execute` (added in Phase 5) | Paginated reads with a `LIMIT ?` placeholder use `pool.query()` rather than `pool.execute()`. | mysql2's prepared-statement path (`execute`) sends a `LIMIT` placeholder value as a string, which MySQL rejects with "Incorrect arguments to mysqld_stmt_execute". `query` performs the same value escaping (parameters passed separately, never concatenated), so it satisfies the parameterized-query rule while accepting `LIMIT`. Applies anywhere a later phase parameterizes `LIMIT`/`OFFSET` — e.g. article lists in Phase 6. |
| D39 | Test timeout (added in Phase 4, refines D29) | Global `testTimeout: 20_000` in `vitest.config.ts`. | With the suite run serially (D29) and every session doing real cost-12 bcrypt hashing, the full run loads the machine enough that the default 5s per-test limit flaked two bcrypt-heavy user tests. 20s absorbs load spikes without masking a genuinely hung test; the subprocess seed tests keep their explicit 60s. |
| D37 | `EMAIL_IN_USE` error code (added in Phase 3) | New 409 code in the D31 taxonomy for a duplicate email on `POST /users`. Detected twice: an explicit lookup for the clear error, and by catching the unique-index violation, which closes the check-to-insert window. | The plan required a 409 for duplicates but D31 had no code covering it. Enumeration is not a concern here — the route is admin-only. |
| D38 | Strict request bodies (added in Phase 3) | All request schemas use `z.strictObject`: an unknown key is a 400, not silently stripped. Applied to the auth schemas at the same time so the convention is uniform. | zod strips unknown keys by default, so `POST /users` with `mustChangePassword: false` or `passwordHash` returned 201 having quietly ignored them. With two first-party clients, being told a field does not exist is strictly better than believing you set it. |
| D35 | Proxy trust (added in Phase 2) | `app.set('trust proxy', 1)` in production only; `false` elsewhere. | Railway terminates TLS and forwards one hop. Without it `req.ip` is the proxy's address, so the entire family shares one rate-limit bucket and 30 failures locks everyone out. Trusting exactly one hop — rather than the whole chain — stops a client spoofing `X-Forwarded-For` to dodge the limit. |
| D36 | Revocation reason (added in Phase 2, refines D7) | `refresh_tokens.revoked_reason` ENUM (`rotated`, `logout`, `password_change`, `admin_reset`, `deactivated`, `theft`), migration 002. Replay of a token revoked by **`rotated`** triggers family-wide revocation; replay of one revoked for any other reason returns 401 and nothing more. | D7 as written treated *any* revoked-token reuse as theft. But a password change revokes every session, so the user's other device would wake up, refresh with its now-dead token, and trigger the theft response — destroying the session created by the change itself. Changing your password would sign you out of the device you changed it on. Only rotation-revoked reuse is genuine evidence of theft. |
| D33 | Timestamp ownership (added in Phase 1) | No `DEFAULT CURRENT_TIMESTAMP` and no `ON UPDATE` anywhere in the schema. The application writes every `created_at`/`updated_at` via `dbNow()`. A migration test asserts no DATETIME column has a default or an `extra` clause. | Article optimistic concurrency (D23) compares `updatedAt` as an exact string. A database-generated value could be written by a path the service does not control, and MySQL's `ON UPDATE` fires only when a row actually changes — a no-op PATCH would leave `updatedAt` untouched and silently break conflict detection. |
| D34 | Runtime assets in the build (added in Phase 1, extended in Phase 2) | `npm run build` runs `tsc` then `scripts/copy-assets.mjs`, which copies `src/db/migrations/` and `src/data/` into `dist/`. A test asserts every non-`.ts` file under `src/` reaches `dist/`, and the server loads the common-password list at boot so a packaging miss crashes startup rather than a user's password change. | `tsc` emits only JavaScript; without the copy, `npm start` finds no migrations and would serve a fresh Railway database against an empty schema. Phase 2 hit the same class again — `common-passwords.txt` was absent from `dist/`, so *every* password change returned 500 in the built artifact while the whole suite passed, because tests run from `src/` via tsx. Hence both the test and the fail-fast boot check. |
...
> **Deferred (D42).** This environment has no AWS credentials or bucket, so the real-bucket round-trip above is carried into Phase 8, where the bucket is created during deployment. Everything not needing a live bucket — signed headers, expiry, the content-type/size/auth/404 paths, and the built-artifact behavior — was verified in Phase 7.

**Gate:** §5 + the manual S3 checklist (the live-bucket portion deferred to Phase 8 per D42).
